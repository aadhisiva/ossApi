...
import { API_VERSION_ISSUE, HEADERS_ISSUE, SUPER_ADMIN } from "./constants";
...
export async function authVersion(req, res, next) {
    ...
    if (!authVersion) return res.status(200).send({ code: 403, status: "Failed", message: "Api Version Not Provided." })
